...
const router = {
    ...
    init() {
        ...
        if (storedPage && this.routes.includes(storedPage)) {
            // If we are logged in, we can proceed to show the stored page
    ...
    showPage(page, pushState = true) {
        ...
        const pageModules = {
            ...
            'absensi': window.absensi,
            'face-recognition': window.faceRecognition,
            'izin': window.izin,
            'jurnal': window.jurnal,
            'cuti': window.cuti,
            'admin-dashboard': window.adminDashboard,
            'employees': window.adminEmployees,
            'attendance-reports': window.adminReports,
            'jurnal-reports': window.adminReports,
            'leave-reports': window.adminReports,
            'shift-schedule': window.shiftSchedule,
            'settings': window.settings
        };

        const targetModule = pageModules[page];
        const alreadyInitialized = targetModule && targetModule.initialized;

        // ONLY show loader if the target module is not yet initialized
        if (typeof loader !== 'undefined' && !alreadyInitialized) {
            loader.show();
        }
