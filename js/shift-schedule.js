...
const shiftSchedule = {
    ...
    async init() {
        try {
            ...
            // Set default selectors to current month/year
            ...
            console.error('Shift schedule init error:', error);
        } finally {
    ...
    updateShift(employeeId, day, shiftValue) {
        ...
        // Auto save to backend real-time
    ...
    triggerAutoSave() {
        ...
        const saveBtn = document.getElementById('btn-save-schedule');
        if (saveBtn) saveBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Auto-saving...';
        ...
        this.autoSaveTimeout = setTimeout(async () => {
            try {
                ...
                if (saveBtn) saveBtn.innerHTML = '<i class="fas fa-save"></i> Tersimpan (Terbaru)';
                setTimeout(() => {
                    if (saveBtn && saveBtn.innerHTML.includes('Tersimpan')) {
                        saveBtn.innerHTML = '<i class="fas fa-save"></i> Simpan Jadwal';
                    }
                }, 2000);
            } catch (error) {
                console.error('Auto-save error', error);
                if (saveBtn) saveBtn.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Gagal Simpan';
            }
